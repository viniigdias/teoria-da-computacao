...
def exibir() -> None:
    ...
    with col1:
        variaveis_input = st.text_input("Variáveis (separadas por vírgula)", value="S", key="input_var")
        simbolo_inicial = st.text_input("Símbolo inicial", value="S", key="input_simbolo")
    with col2:
        terminais_input = st.text_input("Terminais (separados por vírgula)", value="a, b", key="input_term")

    st.markdown("### Regras de Produção")
    producoes_texto = st.text_area("Produções", value="S -> aSb\nS -> λ", height=120, key="input_prod")
    cadeia = st.text_input("Cadeia a derivar", value="aaabbb", key="input_cadeia").strip()

    if st.button("Derivar", key="btn_derivar"):
